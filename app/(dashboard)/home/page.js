...
import { Plus, SquareArrowOutUpRight, Pencil, Trash2, Undo } from "lucide-react"
...
import { startAnalyse, deleteAnalyse, fetchAllAnalyse } from "@/utils/dashboard";
...
const HomePage = () => {
    ...
    const handleRestart = async (id) => {
        try {
            ...
            toast.success("Analysis restarted successfully ! Please wait for the analysis to complete.");
        }
        catch (error) {
                                            ...
                                            <CardHeader className="flex flex-row justify-between items-center p-2">
                                                <CardTitle className="flex flex-col gap-2">
                                                    <div className="flex flex-row gap-6 items-center">
                                                        ...
                                                        <Badge className={"text-xs capitalize " + fetchBadgeColor(analyse.status)}>
                                                            {analyse.status}
                                                        </Badge>
                                                    </div>
                                                ...
                                                <div className="flex flex-row gap-3">
                                                    {analyse.status === 'pending' &&
                                                        <Button size="sm" className="flex flex-row gap-2" variant="outline" onClick={() => handleEdit(analyse.id, analyse.stage)}>
                                                            <Pencil size={16} />
                                                            <span>Edit</span>
                                                        </Button>
                                                    }
                                                    <Button size="sm" variant="destructive" className="flex flex-row gap-2" onClick={() => { setIsDeleteModalOpen(true) }}>
                                                        <Trash2 size={16} />
                                                        <span>Delete</span>
                                                    </Button>
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    ...
                                                                    analyse and remove it from our servers.
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
                                                                <AlertDialogCancel variant="outline" onClick={() => { setIsDeleteModalOpen(!isDeleteModalOpen) }}>Cancel</AlertDialogCancel>
                                                                <AlertDialogAction className="bg-destructive hover:bg-destructive" onClick={() => { handleDelete(analyse.id) }}>Delete</AlertDialogAction>
                                                            </AlertDialogFooter>
                                                        </AlertDialogContent>
                                                    </AlertDialog>
                                                    ...
                                                    {analyse.status === 'failed' && <Button size="sm" className="flex flex-row gap-2" onClick={() => { handleRestart(analyse.id) }}>
                                                        <Undo size={16} />
                                                        <span>Restart</span>
                                                    </Button>
                                                    }
