...
const FinancialPage = () => {
    ...
    const overallFinanciallySuitableProposal = selectedFinancialAnalyseData?.overallFinanciallySuitableProposal;
    const proposalAnalyse = selectedFinancialAnalyseData?.proposalAnalyse;
    const reasonForFinancialSelection = selectedFinancialAnalyseData?.reasonForFinancialSelection;
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Explore financial analysis details to make informed decisions.
                </CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <fieldset className="grid gap-6 rounded-lg border p-4">
                            ...
                            <div className="flex flex-col gap-3 text-sm">
                                <div className="flex flex-row gap-1 w-full">
                                    <span className="font-semibold w-2/12">Average Quotion Value</span>
                                    <span className="w-10/12">{financialAnalyse.avgQuote}</span>
                                </div>
                                <div className="flex flex-row gap-1">
                                    <span className="font-semibold w-2/12">Maximum Quotation Value</span>
                                    <span className="flex flex-row w-10/12">{financialAnalyse.maxQuote} -
                                        <span className="flex flex-row gap-2 items-center cursor-pointer hover:underline hover:text-primary capitalize" onClick={() => { handleSectionScroll(`proposal${financialAnalyse.maxQuoteProposal + 1}`) }}>
                                            {fetchProposalCompanyName(financialAnalyse.maxQuoteProposal)}
                                            <SquareArrowOutUpRight size={14} className="text-primary" />
                                        </span>
                                    </span>
                                </div>
                                <div className="flex flex-row gap-1">
                                    <span className="font-semibold w-2/12">Minimum Quotation Value</span>
                                    <span className="flex flex-row w-10/12">{financialAnalyse.minQuote} -
                                        <span className="flex flex-row gap-2 items-center cursor-pointer hover:underline hover:text-primary capitalize" onClick={() => { handleSectionScroll(`proposal${financialAnalyse.maxQuoteProposal + 1}`) }}>
                                            {fetchProposalCompanyName(financialAnalyse.minQuoteProposal)}
                                            <SquareArrowOutUpRight size={14} className="text-primary" />
                                        </span>
                                    </span>
                                </div>
                            ...
                            <div className="flex flex-col gap-3 text-sm">
                                <div className="flex flex-row gap-1 w-full items-start">
                                    ...
                                    <span className="w-10/12">
                                        <span className='flex flex-col gap-2 justify-start'>
                                            <span className="flex flex-row gap-2 items-center cursor-pointer hover:underline hover:text-primary capitalize" onClick={() => { handleSectionScroll(`proposal${overallFinanciallySuitableProposal + 1}`) }}>
                                                {fetchProposalCompanyName(overallFinanciallySuitableProposal)}
                                                <SquareArrowOutUpRight size={14} className="text-primary" />
                                            </span>
                                        </span>
                                    </span>
                                ...
                                <div className="flex flex-row gap-1 w-full items-start">
                                    ...
                                    <span className="flex flex-col gap-1 w-10/12">
                                        {reasonForFinancialSelection.map((reason, idx) => (
                                ...
                                <div className="flex flex-row gap-1 items-start w-full">
                                    ...
                                    <span className="flex flex-col gap-2 w-10/12 capitalize">
                                        {financialRanking.map((proposal, idx) => (
                        ...
                        {proposalAnalyse.map((proposal, idx) => (
                            <fieldset key={idx} id={`proposal${idx + 1}`} className="grid gap-6 rounded-lg border p-4">
                                ...
                                <div className="flex flex-col gap-3 text-sm">
                                    ...
                                    <div className="flex flex-row gap-1 w-full">
                                        <span className="font-semibold w-2/12">Financial Analyse</span>
                                        <span className="w-10/12">
                                            {proposal.financialAnalysis.length > 0 ? proposal.financialAnalysis.map((analyse, idx) => (
